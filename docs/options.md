...
Controls the `id` attribute generated on headings. Accepts either `false` (no IDs at all) or an object `{prefix, raw}`.

* `false` — disable heading ID generation entirely.
* `{}` (the default) — **GitHub-compatible** IDs: spaces become dashes, most non-alphanumeric characters are stripped, and the result is lower-cased. No prefix.
* `prefix` (string) — prepended to every generated ID, e.g. `{prefix: 'section-'}`. Useful to avoid clashes with pre-existing IDs on the page.
* `raw: true` — minimal sanitization instead of the GitHub-compatible style: only ` ` (space), `'`, `"`, `>` and `<` are replaced with `-` (dash), including in any prefix. All other characters are kept verbatim (and the result is lower-cased).
* `prefix` and `raw` can be combined, e.g. `{prefix: '/p/', raw: true}`.

!!! danger ""
    **Use `raw: true` with caution** as it might result in malformed IDs.

!!! note ""
    This option replaces the removed `noHeaderId`, `prefixHeaderId` and `rawHeaderId` options. Migrate as: `noHeaderId: true` → `headerIds: false`; `prefixHeaderId: 'foo'` → `headerIds: {prefix: 'foo'}` (and `prefixHeaderId: true` → `headerIds: {prefix: 'section-'}`); `rawHeaderId: true` → `headerIds: {raw: true}`.
...
=== "output (value is `{prefix: 'section-'}`)"
